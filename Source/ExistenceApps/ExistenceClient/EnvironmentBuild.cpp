...
int EnvironmentBuild::GenerateWorldObjects(const time_t &timeseed,  terrain_rule terrainrule)
{
    ...
    case WORLD_TERRAIN:
        ...
        for(unsigned int i=0; i<600; i++)
        {
            ...
            CreateObjectsAlongPath(EnvironmentBuild_TREES, patchWorldSize.x_, patchWorldSize.y_,randomSpotx,randomSpotz, 6, 100.0f, terrainHeightMap);
        }
        break;
    default:
        break;
...
int EnvironmentBuild::CreateRockObjectAlongPath( float x, float z, float numberofobjects, float length)
{
    /// Get Needed SubSystems
    ResourceCache* cache = GetSubsystem<ResourceCache>();
    Renderer* renderer = GetSubsystem<Renderer>();
...
/// Old code
/// Create rocks along a path
int EnvironmentBuild::CreateTreeObjectAlongPath(float worldsize_x, float worldsize_y, float x, float z, float numberofobjects, float length, Image * terrainHeightMap)
{
    /// Get Needed SubSystems
    ResourceCache* cache = GetSubsystem<ResourceCache>();
    Renderer* renderer = GetSubsystem<Renderer>();
...
int EnvironmentBuild::CreateObjectsAlongPath(int objecttypes, float worldsize_x, float worldsize_y, float x, float z, float numberofobjects, float length, Image * terrainHeightMap)
{
    /// Get Needed SubSystems
    ResourceCache* cache = GetSubsystem<ResourceCache>();
    Renderer* renderer = GetSubsystem<Renderer>();
...
int EnvironmentBuild::GenerateGrass2(float worldsize_x, float worldsize_y, Image * terrainHeightMap)
{
